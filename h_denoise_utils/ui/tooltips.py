...
AOVS_INPUT = "Select which AOV layers to denoise; use the field below for custom names"
...
ALBEDO_COMBO = "Albedo AOV name; improves quality when present"
...
TEMPORAL_CHK_ENABLED = "Use previous frame for temporal denoising (OptiX only)"
TEMPORAL_CHK_BACKEND_UNSUPPORTED = "Temporal denoising not supported by {}"
TEMPORAL_CHK_NO_MOTION = "Requires motion vectors AOV to enable temporal denoising"
DENOISER_COMBO = "Bundled denoiser executable"
CUSTOM_EXE_BTN = "Custom executables are not used by this bundled OptiX build"
EXRMODE_COMBO = "HOUDINI_OIIO_EXR read mode (-1, 0, or 1); default uses env var"
OPTIONS_EDIT = "Advanced denoiser options are not exposed in this bundled OptiX build"
...
def planes_toggle(count: int, timestamp: str) -> str:
...
def temporal_backend_unsupported(backend_display: str) -> str:
...
def options_invalid_json(exc: object) -> str:
...
def output_destination_label(preview_path: str) -> str:
...
def action_destination_label(preview_path: str) -> str:
