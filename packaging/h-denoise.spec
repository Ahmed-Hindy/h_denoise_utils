...
VENDOR_OIDN_DENOISER_DIR = PACKAGE_DIR / "vendor" / "oidn-denoiser"
PACKAGE_VARIANT = os.environ.get("HDU_PACKAGE_VARIANT", "").strip().lower()
...
if VENDOR_OIDN_DENOISER_DIR.exists() and PACKAGE_VARIANT in ("", "bundled"):
    datas.extend(
        (
            str(path),
            str(Path("h_denoise_utils") / "vendor" / "oidn-denoiser" / path.relative_to(VENDOR_OIDN_DENOISER_DIR).parent),
        )
        for path in VENDOR_OIDN_DENOISER_DIR.rglob("*")
        if path.is_file()
    )
